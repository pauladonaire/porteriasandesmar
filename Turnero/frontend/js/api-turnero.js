...
// Depósito activo (MZA/BUE) para las páginas de turnero. Prioridad: elección manual
// guardada en este navegador (localStorage) > columna Deposito_Turnero del usuario en
// USUARIOS (si es MZA o BUE — los admin tienen "TODOS" ahí, no cuenta como default) > MZA.
const TurneroDeposito = {
  KEY: 'ip_turnero_deposito',
  obtener() {
    try {
      const guardado = localStorage.getItem(this.KEY);
      if (guardado === 'MZA' || guardado === 'BUE') return guardado;
    ...
    return (dep === 'MZA' || dep === 'BUE') ? dep : 'MZA';
  },
  fijar(dep) {
